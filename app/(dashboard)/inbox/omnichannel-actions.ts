...
function getConversationAuditLabel(conversation: {
  id: string;
  customer_name?: string | null;
}) {
  return conversation.customer_name?.trim() || conversation.id;
}
...
export async function assignOmnichannelConversation(
  ...
  try {
    ...
    await auditFromProfile(supabase, profile, {
      ...
      entityLabel: getConversationAuditLabel(conversation),
      metadata: {
...
export async function convertOmnichannelConversationToLead(
  formData: FormData,
  ...
  try {
    ...
    const defaultNotes =
      ...
      buildInboxConvertNotesDefault([
        ...(firstIncomingMessage?.message_text
          ? [
              {
                direction: "incoming",
                message_text: firstIncomingMessage.message_text,
              },
            ]
          : []),
        ...(lastIncomingMessage?.message_text &&
        lastIncomingMessage.message_text !== firstIncomingMessage?.message_text
          ? [
              {
                direction: "incoming",
                message_text: lastIncomingMessage.message_text,
              },
            ]
          : []),
      ]) ||
    ...
    const channel = conversation.channel as "instagram" | "facebook" | "whatsapp";
    ...
    const { data: createdLead, error: leadError } = await supabase
      ...
      .insert({
        ...
        source: mapOmnichannelChannelToLeadSource(channel),
        ...
        metadata: {
          ...
          omnichannel_channel: conversation.channel,
          ai_extraction: null,
        },
      })
      .select("id")
    ...
    await supabase.from("lead_activities").insert({
      ...
      body: `Lead created from ${conversation.channel} conversation.`,
      metadata: {
        omnichannel_conversation_id: conversation.id,
      },
    });
...
export async function createInboxFollowUpTask(
  formData: FormData,
  ...
  try {
    ...
    await supabase.from("lead_activities").insert({
      ...
      metadata: {
        ...
        omnichannel_conversation_id: conversationId,
        source: "inbox",
      },
    });
