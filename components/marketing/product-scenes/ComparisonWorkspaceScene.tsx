      ...
      <div className={cn(sceneStyles.canvas, "m-2 grid gap-3 p-3 sm:m-3 sm:grid-cols-2 sm:items-stretch sm:p-4")}>
        <ScenePanel padding="compact" title={copy.comparison.disconnected} className={sceneStyles.secondarySurface}>
          <div className="grid gap-2 sm:grid-cols-2">
            {[
              { icon: MessageSquare, label: "WA · Thread A" },
              { icon: Table, label: locale === "id" ? "Spreadsheet" : "Spreadsheet" },
              { icon: StickyNote, label: locale === "id" ? "Catatan manual" : "Manual notes" },
            ].map(({ icon: Icon, label }) => (
              <div key={label} className={cn(sceneStyles.mutedSurface, "p-2 opacity-80")}>
