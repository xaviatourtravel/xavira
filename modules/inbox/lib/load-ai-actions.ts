...
export function mapAiActionRows(
...
function mapRow(row: AiActionRow, documentName: string | null): InboxAiActionItem {
  ...
  return {
    ...
    metadata: parseRetryMetadata(row.metadata),

    scheduledFor: row.scheduled_for,

    executedByJob: row.executed_by_job ?? false,
...
export async function loadConversationAiActions(
  ...
  return mapAiActionRows(rows, documentNames);
