-- Add lead_date for actual lead arrival date (separate from CRM created_at audit field)
...
CREATE INDEX leads_organization_id_lead_date_idx
  ON public.leads (organization_id, lead_date DESC);
...
WITH lead_days AS (
  SELECT
    ...
    COALESCE(
      l.lead_date,
      (l.created_at AT TIME ZONE COALESCE(o.timezone, 'Asia/Jakarta'))::date
    ) AS metric_date,
    l.id AS lead_id,
    l.status,
    l.budget_idr,
